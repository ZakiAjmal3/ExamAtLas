...
public class AdminShowAllSubCategoryAdapter extends RecyclerView.Adapter<AdminShowAllSubCategoryAdapter.ViewHolder> {
    ...
    public void onBindViewHolder(@NonNull AdminShowAllSubCategoryAdapter.ViewHolder holder, @SuppressLint("RecyclerView") int position) {
        ...
        holder.setHighlightedText(holder.title, currentCategory.getSubCategoryName() + ":", currentQuery);
        holder.setHighlightedText(holder.categoryTxt, currentCategory.getCategoryName(), currentQuery);
        ...
        String imageURL = subCategoryModelArrayList.get(position).getImageURL();
        Log.e("imageUrl",imageURL);
        Glide.with(context)
                        .load(imageURL)
                                .error(R.drawable.noimage)
                                        .into(holder.subCatImgV);
    ...
    private void showThreeDotsOptions(AdminShowAllSubCategoryModel currentCategory, int position) {
    ...
    private void choseItems(AdminShowAllSubCategoryModel currentCategory, String selectedItems, int position) {
            ...
            new MaterialAlertDialogBuilder(context.getContext())
                    ...
                    .setPositiveButton("DELETE", new DialogInterface.OnClickListener() {
                        ...
                        public void onClick(DialogInterface dialogInterface, int i) {
                            ...
                            progressDialog.show();
                            deleteSubCategory(subCategoryModelArrayList.get(position),position);
                        }
    ...
    private void deleteSubCategory(AdminShowAllSubCategoryModel adminShowAllCategoryModel, int position) {
        String id = adminShowAllCategoryModel.getSubCategoryId();
        String deleteURL = Constant.BASE_URL + "v1/sub-category/" + id;
    ...
    private void openEditCategoryDialogBox(AdminShowAllSubCategoryModel currentCategory, int position) {
        ...
        subCategoryImgView = dialog.findViewById(R.id.uploadImage);
        Glide.with(context)
                .load(currentCategory.getImageURL())
                .error(R.drawable.noimage)
                .into(subCategoryImgView);

        ((AdminCreateSubCategoryFragment) context).setupCategorySpinner(categorySpinner,titleEditTxt,currentCategory);
        ...
        submitBtn.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                progressDialog.getWindow().setLayout(WindowManager.LayoutParams.WRAP_CONTENT, WindowManager.LayoutParams.WRAP_CONTENT); // Adjust the size
                progressDialog.show();
                imageFile = ((AdminCreateSubCategoryFragment) context).getImageFile();
                sendUpdateSubjectDetails(currentCategory,titleEditTxt.getText().toString().trim(),dialog,position);
            }
        });
        ...
        subCategoryImgView.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                ((AdminCreateSubCategoryFragment) context).openGallery();
            }
        });
        dialog.show();
...
// Background and animation settings
    ...
    private void sendUpdateSubjectDetails(AdminShowAllSubCategoryModel currentCategory, String name, Dialog dialog, int position) {
        String updateURL = Constant.BASE_URL + "v1/sub-category";

        // Prepare form data
        Map<String, String> params = new HashMap<>();
        params.put("id", currentCategory.getSubCategoryId());
        params.put("categoryId", currentCategory.getCategoryId());
        params.put("name", name);

        // Create a Map for files (if there are any files, e.g., images)
        ...
        MultipartRequest multipartRequest = new MultipartRequest(updateURL, params, files,
                    ...
                    public void onResponse(String response) {
                        try {
                            ...
                            if (status) {
                                ...
                                String imageURL = dataObj.getJSONObject("imageUrl").getString("url");

                                // Update the list with new data
                                subCategoryModelArrayList.get(position).setSubCategoryName(name);
                                subCategoryModelArrayList.get(position).setImageURL(imageURL);

                                notifyItemChanged(position);
                                dialog.dismiss();
                                progressDialog.dismiss();
                            } else {
                                Toast.makeText(context.getContext(), "Failed to update subject", Toast.LENGTH_SHORT).show();
                            }
                        } catch (JSONException e) {
                ...
                new Response.ErrorListener() {
                    ...
                    public void onErrorResponse(VolleyError error) {
                        ...
                        Toast.makeText(context.getContext(), errorMessage, Toast.LENGTH_LONG).show();
                        Log.e("SubjectUpdateError", errorMessage);
                    }
                }, authToken);
    ...
    public class ViewHolder extends RecyclerView.ViewHolder {
        ...
        public ViewHolder(@NonNull View itemView) {
            ...
            subCatImgV = itemView.findViewById(R.id.imgBlog);
